...
//        unset($member_level[3]);
    ...
    public function getPayName($payArr,$id)
    {
        foreach ($payArr as $item) {
            if ($id == $item['id']) {
    ...
    public function recharge_do()
    {
        ...
        if ($type == 1) {
            ...
            if( $price>0 ) {
                $mark = '会员充值,管理员手动确认，管理员ID：'.session('user.id');
            ...
            if( $res ) {
                //
                M('recharge')->where(array('id'=>$id))->setField('is_pay',1);
            ...
            $pay_model->pay_vip_success($id, 'admin', session('user.username') . date('YmdHis'));
            $this->success('充值成功');
        }
    }
    ...
    public function recharge()
    {
        if( IS_POST ) {
            ...
            $pays = m('pay_type')->where('status',1)->field('id,pay_name')->select();
            ...
            $list = $model->alias('a')->join(C('DB_PREFIX').'member as c on a.member_id = c.id','left')
                ...
                ->select();
            foreach( $list as &$_list ) {
                $_list['type_text'] = '<font class="label label-default radius" color="#5eb95e">余额充值</font>';
                $_list['cz_type'] = 'C';
                if ( substr($_list['order_no'],0,4) == 'VIPC'){
                    $_list['type_text'] = '<font class="label label-success radius" color="#5eb95e">会员升级</font>';
                    $_list['cz_type'] = 'VIPC';
                }
                $_list['payment_type'] =$this->getPayName($pays,$_list['payment_type']);
                $_list['pay_text'] = '<font class="label label-default radius" color="#5eb95e">未支付</font>';
                if ($_list['is_pay']){
                    $_list['pay_text'] = '<font class="label label-success radius" color="#5eb95e">已支付</font>';
                }
