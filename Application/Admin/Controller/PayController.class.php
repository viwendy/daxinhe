...
class PayController extends AdminBaseController{
    ...
    public function paytype()
    {
        ...
        //列表数据
        $map = $this->_search();
        $list = $this->_list ('pay_type', $map, "*",'order_number' );
    ...
    public function order(){
        ...
        if ($result) {
            $this->success('排序成功',U('Admin/Pay/paytype'));
        }else{
            $this->error('排序失败');
        }
    }
    ...
    public function orderData($data,$id='id',$order='order_number'){
        foreach ($data as $k => $v) {
            $v=empty($v) ? null : $v;
            M('pay_type')->where(array($id=>$k))->save(array($order=>$v));
        }
        return true;
    }
