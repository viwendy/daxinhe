...
class PublicController extends PublicBaseController
{
    ...
    public function getcode()
    {
        ...
        $type = C('SMS_TYPE'); //短信发送方式  短信宝 1  阿里云2 聚合短信 3
        $this->sms($type,$tel,$code); //发送短信
...
//            echo sp_encry($password);exit;
    ...
    //发送短信相关
    public function sms($type,$tel,$code)
    {
        //短信宝
        switch ($type) {
            case 1:
                //var_dump(C('SMS_BAO'));die;
                ...
                $user = C('SMS_BAO')['user']; //短信平台帐号15196952584
                $pass = md5(C('SMS_BAO')['password']);   //短信平台密码
                $content = "【".C('SMS_BAO')['sign']."】您的验证码为{$code}，验证码5分钟内有效。";
            ...
            case 2:
               // ----------阿里云---------------------------------
                ...
                $Alisms = new \Alisms(C('SMS_ALISMS')['AccessKey'], C('SMS_ALISMS')['AccessSecret'], C('SMS_ALISMS')['sign'], C('SMS_ALISMS')['template_id']);
            ...
            case 3:
                ...
                $smsConf = array(
                    'key' => C('SMS_JUHESMS')['key'], //您申请的APPKEY
                    'mobile' => $tel, //接受短信的用户手机号码
                    'tpl_id' => C('SMS_JUHESMS')['tpl_id'], //您申请的短信模板ID，根据实际情况修改
                    //'tpl_value' => '#code#=' . $code //您设置的模板变量，根据实际情况修改
                    'tpl_value' => C('SMS_JUHESMS')['tpl_value'].'=' . $code //您设置的模板变量，根据实际情况修改
                );
